...
| Errors | Health response disclosed connection address/port when DB stopped | Injected database failure returns only 503 unavailable; internal audit failure returns generic 500 | before/health-db-offline.json; after/security-tests.json |
...
| Secrets | Public disposable defaults in current baseline and historical source | Unique local secrets, excluded source/build context; no exact active-secret match in scoped working files or reachable history | after/secret-history-check.json |
